...
dayHostCount = dayGroupedHosts.map(lambda x: (x[0][0], 1)).reduceByKey(lambda x,y: x+y)
...
dayAndHostTuple = access_logs.map(lambda log: (log.date_time.day, 1)).reduceByKey(lambda a, b: a+ b).sortByKey().join(dailyHosts)
groupedByDay = dayAndHostTuple.map(lambda (a,b): (a, b[0]/b[1]))
...
badUniqueEndpoints = badEndpoints.reduceByKey(lambda a, b : a + b)
