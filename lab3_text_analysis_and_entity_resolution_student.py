...
def simpleTokenize(string):
    ...
    """
    return filter(lambda name: name.strip(),re.split(split_regex ,string.lower()))
...
def tokenize(string):
    ...
    """
    return filter(lambda name: name not in stopwords,simpleTokenize(string))
...
def countTokens(vendorRDD):
    ...
    """
    return sum(vendorRDD.map(lambda (x,y): len(y)).collect())
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.takeOrdered(1, lambda s: -len(s[1]))
...
def tf(tokens):
    ...
    totaln = len(tokens)
    for s in tokens :
        if s in new_dict.keys():
            new_dict[s] += 1.0/totaln
        else :
            new_dict[s] = 1.0/totaln   
    return new_dict
...
# TODO: Replace <FILL IN> with appropriate code
corpusRDD = amazonRecToToken.map(lambda (x,y):(x,tf(y))).fullOuterJoin(googleRecToToken.map(lambda (x,y):(x,tf(y))))
...
tokenCountPairTuple = uniqueTokens.map(lambda x: (x , 1)).reduceByKey(lambda a, b : a + b)
tokenSumPairTuple = tokenCountPairTuple.map(lambda (x,y):(x,N/y))
...
def idfs(corpus):
    ...
    uniqueTokens = corpus.flatMap(lambda (x,y) : list(set(y)))
    tokenCountPairTuple = uniqueTokens.map(lambda x: (x , 1)).reduceByKey(lambda a, b : a + b)
    tokenSumPairTuple = tokenCountPairTuple.map(lambda (x,y):(x,float(N)/y))
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = tf(tokens)
    tfIdfDict = {k: tfs[k] *  idfs[k]  for k in tfs if k in idfs}
    return tfIdfDict
...
def norm(a):
    ...
    """
    return math.sqrt(sum({k: a[k] *  a[k]  for k in a}.values()))
...
def cosineSimilarity(string1, string2, idfsDictionary):
    ...
    """
    w1 = tfidf(simpleTokenize(string1), idfsDictionary)
    w2 = tfidf(simpleTokenize(string2), idfsDictionary)
    return cossim(w1, w2)
...
avgSimDups = sum(trueDupsRDD.collect())/float(trueDupsCount)

nonDupsRDD = (sims
              .subtractByKey(goldStandard))
avgSimNon = sum(nonDupsRDD.map(lambda (x, y) : y).collect())/float(nonDupsRDD.count())
